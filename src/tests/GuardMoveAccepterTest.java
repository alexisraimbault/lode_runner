...
import contract.algorithms.GuardMoveAccepterContract;
...
public class GuardMoveAccepterTest{
	private GuardMoveAccepterContract commandAccepter;
	IGameState state1;
	IGameState state2;
	IGameState state3;
	IGameState state4;
	...
	public void beforeTests() {
		commandAccepter = new GuardMoveAccepterContract(new GuardMoveAccepter());
	...
	public void acceptTest1() {
		Set<MoveType> acceptedMoves = commandAccepter.accept( state1.getPool().getGuards().get(0));
	...
	public void acceptTest2() {
		Set<MoveType> acceptedMoves = commandAccepter.accept(state2.getPool().getGuards().get(0));
		assertTrue(acceptedMoves.contains(MoveType.RIGHT));
	...
	public void acceptTest3() {
		Set<MoveType> acceptedMoves = commandAccepter.accept( state3.getPool().getGuards().get(0));
		assertTrue(acceptedMoves.contains(MoveType.LEFT));
		assertTrue(acceptedMoves.contains(MoveType.RIGHT));
	...
	public void acceptTest4() {
		Set<MoveType> acceptedMoves = commandAccepter.accept(state4.getPool().getGuards().get(0));
		assertTrue(acceptedMoves.size() == 0);
	}
	
	@Test
	public void acceptTest5() {
		Set<MoveType> acceptedMoves = commandAccepter.accept( state5.getPool().getGuards().get(0));
		assertTrue(acceptedMoves.contains(MoveType.DOWN));
		assertTrue(acceptedMoves.size() == 1);
	}
}