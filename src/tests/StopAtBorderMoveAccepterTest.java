...
import contract.algorithms.StopAtBorderMoveAccepterContract;
import model.algorithms.HookingMoveAccepter;
import model.algorithms.StopAtBorderMoveAccepter;
...
public class StopAtBorderMoveAccepterTest {
	private StopAtBorderMoveAccepterContract<IPlayer> commandAccepter;
	IGameState state1;
	IGameState state2;
	IGameState state3;
	IGameState state4;
	...
	public void beforeTests() {
		commandAccepter = new StopAtBorderMoveAccepterContract(new StopAtBorderMoveAccepter<IPlayer>());
		
		int nb_lives = 3;
		
		EditableEnvironment env1 = new EditableEnvironment(new DynamicScreen());//in the middle
		...
		EditableEnvironment env2 = new EditableEnvironment(new DynamicScreen());//on the extreme left
		env2.resize(width, height);
		for(int i = 0; i<width; i++){
			env2.setCellNature(i, 0, Nature.METAL);
		}
		env2.getCellContent(0, 2).add(EntityType.PLAYER);
		...
		EditableEnvironment env3 = new EditableEnvironment(new DynamicScreen());//On the extreme right
		env3.resize(width, height);
		for(int i = 0; i<width; i++){
			env3.setCellNature(i, 0, Nature.METAL);
		}
		env3.getCellContent(width - 1, 2).add(EntityType.PLAYER);
		...
		EditableEnvironment env4 = new EditableEnvironment(new DynamicScreen());//on the top
		env4.resize(width, height);
		for(int i = 0; i<width; i++){
			env4.setCellNature(i, 0, Nature.METAL);
		}
		env4.getCellContent(8, height - 1).add(EntityType.PLAYER);
		...
		EditableEnvironment env5 = new EditableEnvironment(new DynamicScreen());//topLeft Corner
		env5.resize(width, height);
		for(int i = 0; i<width; i++){
			env5.setCellNature(i, 0, Nature.METAL);
		}
		env5.getCellContent(0, height - 1).add(EntityType.PLAYER);
